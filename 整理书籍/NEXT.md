...
> 最近更新：2026-06-09，commit `web-0.9.8`

## 一句话现状

NAS 运行 `web-0.9.8`（http://192.168.1.44:8766/），书库 39751 本，功能完整。
...
| web-0.9.1 | EPUB/MOBI 阅读器：手机左右滑动翻页 + 进度条移至底部固定栏 |
| web-0.9.7 | 修复 PDF 阅读器只显示封面无法翻页的 bug：iframe 改为 PDF.js canvas 渲染（翻页/缩放/进度条/进度保存/键盘/手机滑动） |
| web-0.9.8 | PDF 阅读器补齐 EPUB 同款触控：点击左右半屏翻页 + 上下/左右滑动翻页 + 整页自适应屏幕 + 顶栏自动隐藏 |
